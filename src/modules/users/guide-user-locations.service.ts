...
@Injectable()
export class GuideUserLocationsService implements GuideUserLocationsServiceInterface {
  constructor(
  ...
  private async validateLocationAndUser({ alias, id }: {alias: string, id: number}): Promise<User> {
    const location = await this.locationRepository.findOne({ alias });

    if(!location) throw new HttpException("Can't find location with this alias.", HttpStatus.CONFLICT);

    const user = await this.usersRepository.findOne({ id });

    if(!user.isHost) throw new HttpException("You can't do this, guides only.", HttpStatus.CONFLICT);
  ...
  async add(alias: string, id: number): Promise<User> {
    ...
    if(verifyIfIsNotNull && user.locationAliasArray.find((aliasStr) => aliasStr === alias)) {
      throw new HttpException("You already joined to this location.", HttpStatus.UNAUTHORIZED);
    }
    
    aliasArray.push(alias);
    await this.usersRepository.nativeUpdate({ id }, { locationAliasArray: aliasArray });
  ...
  async remove(alias: string, id: number): Promise<User> {
    ...
    const aliasIndex = user.locationAliasArray.findIndex((aliasItem) => aliasItem === alias);

    if(aliasIndex !== -1) {
      user.locationAliasArray.splice(aliasIndex,1);
    }
    
    await this.usersRepository.nativeUpdate({ id }, { locationAliasArray: user.locationAliasArray });
    return await this.usersRepository.findOne({ id });
  }

  async findAll(alias: string): Promise<User[]> {
    return await this.usersRepository.find({ locationAliasArray: { $contains: [alias] }, isHost: true, isActive: true });
  }
}