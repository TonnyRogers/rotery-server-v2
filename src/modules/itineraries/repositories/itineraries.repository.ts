...
export class ItineraryRepository extends EntityRepository<Itinerary> {
  private wherePopulateBuilder(
    relationName: string,
    wherePopulate?: FilterQuery<Itinerary>,
  ) {
  ...
  public async findOneQB(
    where: FilterQuery<Itinerary>,
    populate?: string[],
    wherePopulate?: FilterQuery<Itinerary>,
  ) {
    const queryBuilder = this._em.createQueryBuilder(
      Itinerary,
      undefined,
      'read',
    );
    queryBuilder.select(['*']).where(where);

    populate &&
      populate.forEach((popString) => {
        ...
        popString.split('.').forEach((popItem, index, arr) => {
          ...
          if (arr.length > 1 && index !== 0) {
            ...
            previusRelationAlias = `${previusRelationAlias}${popItem.slice(
              0,
              2,
            )}`;
          }
