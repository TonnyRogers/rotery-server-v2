...
export class ItinerariesService {
  ...
  async create(authUserId: number, createItineraryDto: CreateItineraryDto) {
    try {
      ...
      const userSubcription = await this.subscriptionsService.isValid(
        authUserId,
      );
  ...
  async show(id: number) {
    try {
      const itinerary = await this.itineraryRepository.findOneQB(
        { id, deletedAt: null },
        itineraryRelations,
    ...
    try {
      ...
      if (itinerary.members.length > updateItineraryDto.capacity) {
        throw new HttpException(
          "Can't set capacity less than members list.",
      ...
      const formatedTransports = updateItineraryDto.transports.map((item) => ({
        ...item,
        itinerary: itineraryId,
      }));
      const formatedActivities = updateItineraryDto.activities.map((item) => ({
        ...item,
        itinerary: itineraryId,
      }));
      const formatedLodgings = updateItineraryDto.lodgings.map((item) => ({
        ...item,
        itinerary: itineraryId,
      }));
      const formatedPhotos = updateItineraryDto.photos.map((item) => ({
        ...item,
        itinerary: itineraryId,
      }));

      // mudar para promise.all
      await this.itineraryTransportRepository.insertJoinTable(
        { itinerary: itineraryId },
        formatedTransports,
      );
      await this.itineraryActivityRepository.insertJoinTable(
        { itinerary: itineraryId },
        formatedActivities,
      );
      await this.itineraryLodgingRepository.insertJoinTable(
        { itinerary: itineraryId },
        formatedLodgings,
      );
      await this.itineraryPhotoRepository.insertJoinTable(
        { itinerary: itineraryId },
        formatedPhotos,
      );
  ...
  async delete(authUserId: number, itineraryId: number) {
    try {
      ...
      await this.itineraryRepository.populate(
        itinerary,
        ['members.user', 'members.paymentId'],
        {
          where: { members: { deletedAt: null } },
        },
      );
  ...
  async finish(authUserId: number, itineraryId: number) {
    try {
      ...
      await this.itineraryRepository.populate(
        itinerary,
        ['owner', 'members', 'members.user.email', 'members.paymentAmount'],
        {
  ...
  async createValidation(authUserId: number) {
    try {
      const countItineraries = await this.itineraryRepository.count({
        owner: authUserId,
        deletedAt: null,
      ...
      throw new HttpException(
        'Error on validade creation try again.',
