...
export class Migration20211005012247 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      'alter table "itinerary_transport" add constraint "itinerary_transport_itinerary_id_foreign" foreign key ("itinerary_id") references "itinerary" ("id") on update cascade on delete cascade;',
    );
    this.addSql(
      'alter table "itinerary_transport" add constraint "itinerary_transport_transport_id_foreign" foreign key ("transport_id") references "transport" ("id") on update cascade on delete cascade;',
    );
  }
